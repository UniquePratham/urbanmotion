  ...
  const handleSubmitChangePassword = async () => {
    ...
    if (retailerData) {
      ...
      if (currentPassword && newPassword && confirmNewPassword) {
        if (currentPassword===retailerData.password && newPassword===confirmNewPassword) {
          payload = { email: retailerData.email, password: newPassword };
        }
      }
      try {
      ...
      toast({
        title: "Invalid Retailer Data",
        description: "Unable to process the retailer data.",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
  ...
  return (
    ...
    >
      ...
      <Heading color="#00db00" p={{base:6,md:0}} mb={4} width={{base:"100vw"}} fontSize={{base:"3xl",md:"4xl"}}>
        Welcome,<Box as="br" /> {retailerData?.name || "Retailer"}!
      </Heading>
      ...
      >
        ...
        />
       {retailerData ? (
          <Text mb={2}>{retailerData.name}</Text>
      ):null}
        <Button
          bg="#00db00"
          color="white"
